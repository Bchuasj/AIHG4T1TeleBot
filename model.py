...
# The two parameters are similarity score threshold and top k. The table below shows the various scenarios, leading to whether the chucks will lead towards a prompt with relevant context. Bear in mind that it is not practical to exhaustively try all possible values of k, for example, 1,2,3...to 100 or beyond. You can try 2-3 variants for both of the parameters. For example, you could try 5, 10 and 15 for k. For similarity threshold, the value should range between 0 to 1 (1 being the most selective).
    ...
    template = """You are a helpful chatbot that helps healthcare volunteers with healthcare questions regarding migrant workers. 
